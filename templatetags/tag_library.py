...
dtt = dt.datetime
import jpholiday
from time_sheet.models import Shift, User, Post
...
def work_time(object):
    ...
    count = time_count(start_time, end_time)
    if isinstance(count[0], list):
        work_time = count[0]
    else:
        work_time = count
    work_count = minute_count(
        rest_time, dt.time(hour=work_time[0], minute=work_time[1])
    )
...
def work_time(object_list):
    ...
    for object in object_list:
        ...
        if end_time >= night_time or end_time <= dt.time(hour=5):
            ...
            time_list_new = minute_count(
                rest_time, dt.time(hour=time_list[0][0], minute=time_list[0][1])
            )
            ...
            time_list = time_count(start_time, end_time)
            time_list_new = minute_count(
                rest_time, dt.time(hour=time_list[0], minute=time_list[1])
            )
            time_hour_sum += time_list_new[0]
            time_minutes_sum += time_list_new[1]
            if time_list_new[0] >= 8:
