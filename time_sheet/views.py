...
class Index(ListView):
    ...
    def get_queryset(self):
        query_set = Post.objects.filter(name=self.request.user).order_by("-date")

        return query_set

    def post(self, request):
        _ = Post.objects.filter(name=request.user).update(published=False)
...
class Mypage(ListView):
    ...
    def get_queryset(self):
        query_set = (
            User.objects.filter(username__istartswith=244)
            .order_by("-last_login")
            .exclude(username=244)
        )
...
class IndexPost(mixins.MonthCalendarMixin, ListView):
    # 一覧するモデルを指定 -> `object_list`で取得可能
    template_name = "time_sheet/index_post.html"
    model = Post
...
class Update(UpdateView):
    ...
    def get(self, request, **kwargs):
        if not request.user.is_superuser:
            if Post.objects.get(id=self.kwargs["pk"]).name != request.user:
                return HttpResponse("不正なアクセスです。")
...
class MonthWithFormsCalendar(mixins.MonthWithFormsMixin, generic.FormView):
    ...
    template_name = "time_sheet/month_with_forms.html"
    model = Post
    date_field = "date"
        ...
        for i, row in enumerate(reader):
            if i == 0:
                ...
                if User.objects.filter(username=row[4]).exists():
                    ...
                    end_time_min = int(end_time.split(":")[0]) * 60 + int(
                        end_time.split(":")[1]
                    )
                    start_time_min = int(start_time.split(":")[0]) * 60 + int(
                        start_time.split(":")[1]
                    )
...
class ShiftView(mixins.MonthCalendarMixin, ListView):
    template_name = "time_sheet/shift.html"
    model = Shift
...
class ShiftUpdate(mixins.MonthCalendarMixin, UpdateView):
    template_name = "time_sheet/shift_form.html"
    model = Shift
    ...
    def get(self, request, **kwargs):
        if not request.user.is_superuser:
            if Shift.objects.get(id=self.kwargs["pk"]).name != request.user:
                return HttpResponse("不正なアクセスです。")
        return super().get(request)
...
class ShiftIndex(ListView):
    # 一覧するモデルを指定 -> `object_list`で取得可能
    model = Shift
    template_name = "time_sheet/shift_index.html"
    paginate_by = 5

    def get_queryset(self):
        query_set = Shift.objects.filter(name=self.request.user).order_by("-date")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["date_range"] = ",".join([startdate, enddate])
        context["range"] = "〜".join([startdate, enddate])
        context["objects"] = Shift.objects.filter(name=self.request.user).order_by(
            "-date"
        )
